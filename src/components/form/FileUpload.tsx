...
interface FileUploadProps {
  ...
  accept?: string;
  infoMessage?: React.ReactNode; // เปลี่ยนจาก string[] เป็น React.ReactNode
}
...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  return (
    <div className="mb-6">
      ...
      {/* ข้อความแจ้งเตือนที่รับเป็น JSX */}
      {infoMessage && (
        <div className="flex items-start text-sm text-[#B3B3B3] mb-2">
      ...
      >
        ...
        <span className="text-[#008A90] mt-2 font-medium">เลือกไฟล์อัปโหลด หรือ ลากวาง</span>
        <input
          type="file"
        ...
        />
        <p className="text-[#B3B3B3] text-sm mt-1">ไฟล์ประเภท {fileType} ขนาดไม่เกิน {maxSize}</p>
        {recommendedSize && <p className="text-[#B3B3B3] text-sm mt-1">ขนาดแนะนำ: {recommendedSize}</p>}
      </div>
    </div>
  );
};

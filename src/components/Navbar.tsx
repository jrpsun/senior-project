...
const getMenuIcon = (label: string) => {
  switch (label) {
    ...
    case "ติดตามสถานะการสมัคร":
      return <Image src="/images/Navbar/FollowStatus.svg" alt="EditInfo Icon" width={25} height={25} />
        ;
    case "ชำระค่าสมัคร":
      return <Image src="/images/Navbar/Payment.svg" alt="EditInfo Icon" width={25} height={25} />
        ;
    case "เปลี่ยนรหัสผ่าน":
      return <Image src="/images/Navbar/ChangePassword.svg" alt="EditInfo Icon" width={25} height={25} />
        ;
    case "ออกจากระบบ":
      return <Image src="/images/Navbar/Logout.svg" alt="EditInfo Icon" width={25} height={25} />
        ;
    default:
      return null;
  }
        ...
        {!isPrivacyPolicyPage && (
          <div className="flex items-center relative ml-auto">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="font-medium">จิรภัทร สุวรรณลมัย</span>
            </div>

            {/* Dropdown เมนู */}
            <div className="relative">
              <button onClick={() => setDropdownOpen(true)} className="flex items-center px-3 py-2 bg-white">
                <svg width="15" height="10" viewBox="0 0 19 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M9.2627 9.5L0.262695 0.5H18.2627L9.2627 9.5Z" fill="#1D1B20" />
                </svg>
              </button>
              {dropdownOpen && (
          ...
          <div className="relative">
            ...
            {langDropdownOpen && (
              <div className="absolute right-0 mt-2 w-24 bg-white border rounded-[5px] shadow-lg p-2 z-50" onMouseLeave={() => setLangDropdownOpen(false)}>
                {languages.map((lang) => (
                  <button key={lang} className="block w-full px-4 py-2 text-left hover:bg-gray-100" onClick={() => setLangDropdownOpen(false)}>
                    {lang}
                  </button>
                ))}
              </div>
